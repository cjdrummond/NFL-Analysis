...
Correlation_Matrix_and_Plot <- function(dataframe, columns_for_correlation, filename = "correlation_plot.png"){
...
Factorial_ANOVA_all <- function(dataframe, cols_for_fact_anova, filename = "Factorial_ANOVA_output_all.txt"){
    sink(filename)
    for (metric in cols_for_fact_anova) {
        ...
        png(filename = paste0(metric, "_interaction_plot.png"), width = 1250, height = 600)
        interaction.plot(x.factor = dataframe_no_na$Region, trace.factor = dataframe_no_na$Hall_Of_Fame, response = dataframe_no_na[[metric]], type = "b", fun = mean, fixed = TRUE, leg.bty = "o", xlab = "Region", ylab = metric, main = paste(metric, "by College Region and Hall of Fame Status"))
        dev.off()
...
Factorial_ANOVA <- function(dataframe, cols_for_fact_anova, filename = "Factorial_ANOVA_output.txt"){
    sink(filename)
    for (metric in cols_for_fact_anova) {
        formula <- as.formula(paste(metric, "~ Hall_Of_Fame*Region"))
        factorial_anova <- aov(formula, data = dataframe)
...
ANCOVA_Region_HOF_BMI <- function(dataframe, cols_for_ANCOVA, filename = "ANCOVA_output.txt"){
    sink(filename)
    for (metric in cols_for_ANCOVA) {
        ...
        cat("\n Assumptions Check of no interaction between factors and covariate\n")
        formula_no_int_tst <- as.formula(paste(metric, "~ Hall_Of_Fame + Region + BMI + Hall_Of_Fame:Region + Hall_Of_Fame:BMI + Region:BMI"))
        ancova_no_int_tst <- aov(formula_no_int_tst, data = dataframe_no_na)
        print(summary(ancova_no_int_tst))

        cat("\n Assumptions Check for Homogeneity of Variance\n")
...
Correlation_Matrix_and_Plot(player_data_filtered, cols_for_corr, filename = "original_correlation_plot.png")
...
# Compute the correlation matrix
Correlation_Matrix_and_Plot(new_df_all_regions, cols_for_corr_new, filename = "new_all_correlation_plot.png")
...
total_tackles_retired <- na.omit(new_df_retired_regions[, c("Total_Tackles", "is_offense")])
# Filter out rows with missing or zero values
total_tackles_all <- total_tackles_all[total_tackles_all$Total_Tackles > 0, ]
total_tackles_retired <- total_tackles_retired[total_tackles_retired$Total_Tackles > 0, ]
# Create the histogram
...
sack_inv_results <- Inverse_transform_and_plot(new_df_all_regions_1t, new_df_retired_regions_1t, "Sacks")
...
#### Passes Defended ####
passes_defended_all <- na.omit(new_df_all_regions_2t[, c("Passes_Defended", "is_offense")])
passes_defended_retired <- na.omit(new_df_retired_regions_2t[, c("Passes_Defended", "is_offense")])
...
dim(new_df_all_regions_3t)
new_df_retired_regions_3t <- passes_defended_log_results$df_retired
dim(new_df_retired_regions_3t)

#### def_TDs ####

def_TDs_all <- na.omit(new_df_all_regions_3t[, c("def_TDs", "is_offense")])
def_TDs_retired <- na.omit(new_df_retired_regions_3t[, c("def_TDs", "is_offense")])
...
dim(new_df_all_regions_5t)
dim(new_df_retired_regions_5t)
...
dim(new_df_all_regions_6t)
dim(new_df_retired_regions_6t)
...
dim(new_df_all_regions_7t)
dim(new_df_retired_regions_7t)
...
dim(new_df_all_regions_8t)
dim(new_df_retired_regions_8t)
...
dim(new_df_all_regions_9t)
dim(new_df_retired_regions_9t)
...
dim(new_df_all_regions_10t)
dim(new_df_retired_regions_10t)
...
dim(new_df_all_regions_11t)
dim(new_df_retired_regions_11t)
...
dim(new_df_all_regions_12t)
dim(new_df_retired_regions_12t)
...
dim(new_df_all_regions_13t)
dim(new_df_retired_regions_13t)
...
dim(new_df_all_regions_14t)
dim(new_df_retired_regions_14t)
...
dim(new_df_all_regions_15t)
dim(new_df_retired_regions_15t)
...
dim(new_df_all_regions_16t)
dim(new_df_retired_regions_16t)
####### End of transformations #######

View(new_df_all_regions_16t)
...
Correlation_Matrix_and_Plot(new_df_all_regions_16t, col_for_correl, filename = "correlation_all_players_transformed.png")
...
Correlation_Matrix_and_Plot(offensive_players_all, cols_for_corr_off, filename = "correlation_offensive_players_transformed.png")
...
Correlation_Matrix_and_Plot(defensive_players_all, cols_for_corr_def, filename = "correlation_defensive_players_transformed.png")
...
Correlation_Matrix_and_Plot(offensive_players_retired, cols_for_corr_off, filename = "correlation_offensive_players_retired_transformed.png")
Correlation_Matrix_and_Plot(defensive_players_retired, cols_for_corr_def, filename = "correlation_defensive_players_retired_transformed.png")

Correlation_Matrix_and_Plot(new_df_retired_regions_16t, col_for_correl, filename = "correlation_all_players_retired_transformed.png")
## same correlations as all players but slightly different values so will use retired data for factorial ANOVA and ANCOVA so that the data is more applicable to current hall of fame status
...
cols_for_fact_anova_untran <- c("BMI", "Total_Tackles", "Sacks", "Passes_Defended", "def_TDs", "AverageYrdperINT", "Fumble_recovery_kept_pct", "Forced_Fumbles", "Completion_Percentage", "Yrds_gained_per_Attempt", "Passing_TD_per_Game", "Interceptions_Thrown_per_passing_attempt", "First_downs_per_attempt_percentage", "Been_Sacked", "Passer_Rating", "Yards_per_Reception", "First_down_receptions_percentage", "Receiving_TDs", "Rushing_Yards_per_Attempt", "First_down_rushes", "TDs_per_Rush_Attempt")

Factorial_ANOVA_all(new_df_retired_regions, cols_for_fact_anova_untran, filename = "factorial_ANOVA_all_players_retired.txt")


Factorial_ANOVA(offensive_players_retired, cols_for_corr_off, filename = "factorial_ANOVA_offensive_players_retired_trans.txt")
Factorial_ANOVA(defensive_players_retired, cols_for_corr_def, filename = "factorial_ANOVA_defensive_players_retired_trans.txt")
Factorial_ANOVA_all(new_df_retired_regions_16t, cols_for_fact_anova, filename = "factorial_ANOVA_all_players_retired_trans.txt")


# Perform ANCOVA for each performance metric based on Region, hall of fame status, and BMI of the retired players. I am choosing to look at only the retired players since the collected data is as recent as 2020. To be eligible for hall of fame a player must be retired for 5 years. This will allow the data to be more applicible to current hall of fame status as the players that have retired in 2019 are eligible for the 2024 hall of fame class.
cols_for_ancova_all <- c("Total_Tackles_trans", "Sacks_trans", "Passes_Defended_trans", "def_TDs_trans", "AverageYrdperINT_trans", "Fumble_recovery_kept_pct", "Forced_Fumbles_trans", "Completion_Percentage", "Yrds_gained_per_Attempt_trans", "Passing_TD_per_Game_trans", "Interceptions_Thrown_per_passing_attempt_trans", "First_downs_per_attempt_percentage", "Been_Sacked_trans", "Passer_Rating_trans", "Yards_per_Reception_trans", "First_down_receptions_percentage", "Receiving_TDs_trans", "Rushing_Yards_per_Attempt_trans", "First_down_rushes_trans", "TDs_per_Rush_Attempt_trans")

cols_for_ancova_off <- c("Total_Tackles_trans", "Fumble_recovery_kept_pct", "Completion_Percentage", "Yrds_gained_per_Attempt_trans", "Passing_TD_per_Game_trans", "Interceptions_Thrown_per_passing_attempt_trans", "First_downs_per_attempt_percentage", "Been_Sacked_trans", "Passer_Rating_trans", "Yards_per_Reception_trans", "First_down_receptions_percentage", "Receiving_TDs_trans", "Rushing_Yards_per_Attempt_trans", "First_down_rushes_trans", "TDs_per_Rush_Attempt_trans")

cols_for_ancova_def <- c("Total_Tackles_trans", "Sacks_trans", "Passes_Defended_trans", "def_TDs_trans", "AverageYrdperINT_trans", "Fumble_recovery_kept_pct", "Forced_Fumbles_trans")

## perform ANCOVA for each performance metric by position grouped into offense and defense
ANCOVA_Region_HOF_BMI(offensive_players_retired, cols_for_ancova_off, filename = "ANCOVA_offensive_players_trans.txt")
ANCOVA_Region_HOF_BMI(defensive_players_retired, cols_for_ancova_def, filename = "ANCOVA_defensive_players_trans.txt")
ANCOVA_Region_HOF_BMI(new_df_retired_regions_16t, cols_for_ancova_all, filename = "ANCOVA_all_players_retired_trans.txt")
